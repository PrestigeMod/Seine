...
echo "liblights..."
if [ ! -f /system/.siyah/liblights-installed ];
then
  if [ "$payload_extracted" == "0" ];then
    extract_payload
  fi
...
echo "camera..."
if [ ! -f /system/.siyah/camera-installed ];
then
  if [ "$payload_extracted" == "0" ];then
    extract_payload
  fi
 mv /system/lib/hw/camera.exynos4.so /system/lib/hw/camera.exynos4.so.BAK
 cp /res/misc/camera.exynos4.so /system/lib/hw/camera.exynos4.so
 chown 0.0 /system/lib/hw/camera.exynos4.so
 chmod 644 /system/lib/hw/camera.exynos4.so
 echo 1 > /system/.siyah/camera-installed
fi
